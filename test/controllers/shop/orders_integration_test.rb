...
class Shop::OrdersIntegrationTest < ActionDispatch::IntegrationTest
  ...
  test "confirming an order" do
    # 1. Add item
    post shop_event_order_items_path(@store.slug, @event), params: {event_product_id: @product.id}
    ...
    assert_emails 1 do
      perform_enqueued_jobs do
        post confirm_shop_event_path(@store.slug, @event)
      end
    end
  ...
  test "unconfirming an order" do
    ...
    post unconfirm_shop_event_path(@store.slug, @event)
    assert_redirected_to shop_event_path(@store.slug, @event)
